...
- (void)setUpMenuItemView:(CGFloat)menuItemWidth menuScrollViewHeight:(CGFloat)menuScrollViewHeight indicatorHeight:(CGFloat)indicatorHeight separatorPercentageHeight:(CGFloat)separatorPercentageHeight separatorWidth:(CGFloat)separatorWidth separatorRoundEdges:(BOOL)separatorRoundEdges menuItemSeparatorColor:(UIColor *)menuItemSeparatorColor
{
    ...
    _menuItemSeparator.backgroundColor = [UIColor whiteColor];
...
- (void)initValues
{
    ...
    _menuItemSeparatorPercentageHeight = 0.5;
...
- (void)configureUserInterface
{
    ...
    if (_useMenuLikeSegmentedControl) {
        selectionIndicatorFrame = CGRectMake(10.0, _menuHeight - _selectionIndicatorHeight, self.view.frame.size.width / (CGFloat)_controllerArray.count-20, _selectionIndicatorHeight);
    } else if (_menuItemWidthBasedOnTitleTextWidth) {
...
- (void)moveSelectionIndicator:(NSInteger)pageIndex
{
    if (pageIndex >= 0 && pageIndex < _controllerArray.count) {
        [UIView animateWithDuration:0.15 animations:^{
            ...
            self.selectionIndicatorView.frame = CGRectMake(selectionIndicatorX+10, self.selectionIndicatorView.frame.origin.y, selectionIndicatorWidth-20, self.selectionIndicatorView.frame.size.height);
    ...
    if (tappedPoint.y < self.menuScrollView.frame.size.height) {
        ...
        if (itemIndex >= 0 && itemIndex < _controllerArray.count) {
            ...
            [UIView animateWithDuration:duration animations:^{
                CGFloat xOffset = (CGFloat)itemIndex * self->_controllerScrollView.frame.size.width;
                [self->_controllerScrollView setContentOffset:CGPointMake(xOffset, self->_controllerScrollView.contentOffset.y)];
            }];
...
- (void)viewDidLayoutSubviews
{
    ...
    if ((oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation)) || (!oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation))){
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            _selectionIndicatorView.frame =  CGRectMake(selectionIndicatorX+10, self.selectionIndicatorView.frame.origin.y, selectionIndicatorWidth-20, self.selectionIndicatorView.frame.size.height);
