...
class MicroPhoneVC: UIViewController, AVAudioRecorderDelegate, RecorderDelegate {
    ...
    func setUIElementsProperties() {
        ...
        self.headingLbl.text = self.getLocalizatioStringValue(key: "Checking mic")
        self.headingLbl.font = UIFont.init(name: AppFontBold, size: self.headingLbl.font.pointSize)
        self.subHeadingLbl.text = self.getLocalizatioStringValue(key: "Clck to start button, after that microphone will listen your voice for 4 seconds to check your microphone is working or not")
